...
export default function DraftSpotHero({ model }: { model: DraftSpotViewModel }) {
  ...
  const cards = [
    ...
    ['Last-place danger', hero.saundersPick ? draftPositionLabel(hero.saundersPick.draft_pick, state.normalize) : '—', hero.saundersPick ? `${formatPercent(hero.saundersPick.saunders_rate)} · n=${hero.saundersPick.n}` : 'No sample'],

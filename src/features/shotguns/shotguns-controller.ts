...
function recentCompletedRecord(row: ShotgunRecord): string {
  const playable = Boolean(row.media_key && MEDIA_BASE_URL);
  const label = recordLabel(row, playable ? 'play' : 'unavailable');
  const unavailableCopy = `Media unavailable for ${vivaShotgunDisplayName(row.owner)} · ${row.date} · ${row.cause}`;
  return `<li class="shotgun-record"><div class="shotgun-record-details"><strong>${escapeHtml(vivaShotgunDisplayName(row.owner))}</strong><span>${escapeHtml(row.date)}${row.week ? ` · Week ${escapeHtml(row.week)}` : ''}</span><span>${escapeHtml(row.cause)}</span></div><button class="btn shotgun-play" type="button" data-shotgun-id="${escapeHtml(row.id)}" aria-label="${escapeHtml(label)}" ${playable ? '' : 'disabled'}>${playable ? 'Play clip' : escapeHtml(unavailableCopy)}</button></li>`;
}
...
export function createFeatureController(): VivaFeatureController {
  ...
  const render = () => {
    ...
    const completedOwnerNames = owners.filter(owner => completed.some(row => row.owner === owner)).sort((a, b) => (
      String(completed.find(row => row.owner === b)?.date || '').localeCompare(String(completed.find(row => row.owner === a)?.date || ''))
      || a.localeCompare(b)
    ));
